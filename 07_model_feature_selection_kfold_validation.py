...
def modelValidation(kFold, nNeighbors, dst, methodFeature):
    ...
    clfSelected = make_pipeline(SelectKBest(f_classif, k=50), MinMaxScaler(), KNeighborsClassifier(n_neighbors=int(nNeighbors)))
    
    cvScores = cross_val_score(clfSelected, dfFeature, dfLabel, cv=int(kFold))
